...
class PayslipBatchesBanco(models.Model):
   ...
   #ULTIMOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    ...
    def get_nomi_data_banco(self):
        ...
        worksheet.write(row, 0, 'Número de colaborador', heading_format)
        worksheet.write(row, 1, 'Nombre del colaborador', heading_format)
        worksheet.write(row, 2, 'Proyecto', heading_format)
        worksheet.write(row, 3, 'Número de identidad', heading_format)
        worksheet.write(row, 4, 'Cuenta bancaria', heading_format)
        worksheet.write(row, 5, 'Salario neto a pagar', heading_format)
        ...
        for slip in self.slip_ids:
            numero += 1
            worksheet.write(row, 0, int(numero), cell_text_format)
            worksheet.write(row, 1, str(slip.employee_id.name), cell_text_format)
            worksheet.write(row, 2, str(slip.employee_id.tipo_proyecto_id_pro.nombre_proyecto), cell_text_format)
            worksheet.write(row, 3, str(slip.employee_id.identification_id), cell_text_format)
            worksheet.write(row, 4, str(slip.employee_id.cuenta_bancaria_personal), cell_text_format)  
            ...
            for papo in recorrer_sueldo.line_ids:
                if papo.code == 'NET':
                   ...
                   worksheet.write(row, 5, sueldo_toral, cell_text_format)
                   row += 1    
