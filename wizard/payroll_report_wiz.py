...
class PayslipBatches(models.Model):
        ...
        if self.slip_ids:
            for line in self.env['hr.payslip.line'].search([('slip_id', 'in', self.slip_ids.ids)], order="sequence"):
                ...
                if line.code not in result.keys():
                    if line.code == 'Sueldo Mensual TEMP':
                        continue
                    elif line.code == 'Sueldo Quincenal TEMP':
                        continue
                    else:
   ...
   #ULTIMOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    ...
    def get_nomi_data(self):
        ...
        total_prestamo = 0.0
        total_incapa = 0.0
        total_telefonode = 0.0
        total_cafe1 = 0.0
        total_labora = 0.0
        total_pricesmart = 0.0
        total_uniforme = 0.0 
        total_no_trabajo = 0.0
        ...
        for slip in self.slip_ids:
            ...
            vaca_validacion = self.env['hr.leave'].search([('employee_id', '=', slip.employee_id.id),('state', '=', 'validate')]) 
            ...
            #Dias completos que se trabajaron
            ...
            if contrato_validacion:
               ...
               suelquincenall = (contrato_validacion.wage/2)
            ...
            if ingre_emple:     
                ...
                if hora:
                    tot_ingr = float(sueldo_neto) + float(total_ingre) + float(hora) + float(pre_ingre_dedu_total)
                    va_in = float_round(tot_ingr, precision_digits=2)
                    worksheet.write(row, 20, tot_ingr, cell_number_format) 
            ...
            for ingrep in dedu_ingre_anual:
                ...
                if ingrep.tipo_dedu_id.code == 'BNTRA' and to_ingre4 == 0.0:
                ...
                if ingrep.tipo_dedu_id.code == 'VACA' and to_ingre66 == 0.0:
            ...
            #Validar que el sueldo quincenal solo la quincena que ingresa la primera sea el valor de lo contrario sea su quincena normal

            guardar_sueldo = 0.0
            validar_sueldo_por_ingreso = False
            if sueldo_isre > 0.0 and year_actual == year_mo and month_actual == fee:
                if dia_isr > 1 and dia_isr < 10:             
                    vap = (contrato_validacion.wage / 2)
                    #Validacion que si se le pago una quincena a esa persona en ese mes que valide la resta de lo contrario agarre la mitad
                    sueldo_ac = self.env['hr.employee.sueldos'].search([('employee_id', '=', slip.employee_id.id),('year_sueldo', '=', year_actual),('mes_sueldo', '=', month_actual)])

                    if sueldo_ac:
                       guardar_sueldo = vap
                    else:
                        guardar_sueldo = (vap - sueldo_isre)

                    tot_ingr = guardar_sueldo
                    #Este validacion hace que se escriba el sueldo completo cuando los empleados ingresen a mediados del mes
                    suelquincenall = tot_ingr
                    worksheet.write(row, 12, tot_ingr, cell_number_format) 
                    worksheet.write(row, 20, tot_ingr, cell_number_format) 
                    ...
                    for natu in dedu_emple:
                        for days in range(dias_totales + 1): 
                            ...
                            if natu['fecha_precio'] == fecha:
                                ...
                                if natu.tipo_dedu_id.code == 'PRESTA':
                                    to_descu4 += natu['monto_lps']   
                                if natu.tipo_dedu_id.code == 'INCAPA':
                                    to_descu5 += natu['monto_lps']
                                if natu.tipo_dedu_id.code == 'TELEFO':
                                    to_descu6 += natu['monto_lps']
                                if natu.tipo_dedu_id.code == 'CAFE1':
                                    to_descu7 += natu['monto_lps'] 
                                if natu.tipo_dedu_id.code == 'LABO':
                                    to_descu8 += natu['monto_lps']
                                if natu.tipo_dedu_id.code == 'PRICE':
                                    to_descu9 += natu['monto_lps']
                                if natu.tipo_dedu_id.code == 'UNIFO':
                                    to_descu10 += natu['monto_lps']
                                if natu.tipo_dedu_id.code == 'OTRAS':
                                    to_descu11 += natu['monto_lps']
                                # if natu.tipo_dedu_id.code == 'NOTRABAJO':
                                #     to_descu12 += natu['monto_lps']
                        #IHSS    
                        ...
                        worksheet.write(row, 23, pal_final, cell_number_format)
                        #IMPUESTO VECINAL
                        worksheet.write(row, 24, to_descu3, cell_number_format)
                        #PRESTAMO
                        worksheet.write(row, 25, to_descu4, cell_number_format)
                        #INCAPACIDAD
                        worksheet.write(row, 26, to_descu5, cell_number_format)
                        #TELEFONO
                        worksheet.write(row, 27, to_descu6 , cell_number_format)
                        #CAFETERIA1
                        worksheet.write(row, 28, to_descu7, cell_number_format)
                        #PRUEBA_LABO
                        worksheet.write(row, 29, to_descu8, cell_number_format)
                        #PRICESMART
                        worksheet.write(row, 30, to_descu9, cell_number_format)
                        #UNIFORME
                        worksheet.write(row, 31, to_descu10, cell_number_format)
                        #OTRAS DEDUCCIONES
                        worksheet.write(row, 32, to_descu11, cell_number_format)
                        #DIAS NO TRABAJO
                        sueldo_diarioooo = (contrato_validacion.wage / 30)
                        to_descu12 = ( sueldo_diarioooo * inpa)
                        worksheet.write(row, 33, to_descu12, cell_number_format)

                        suma_de = float(total_dedu) + float(pal_final) + float(to_descu1 + to_descu2) + float(to_descu12)
                        va_dedu = float_round(suma_de, precision_digits=2)
                        worksheet.write(row, 34, va_dedu, cell_number_format)
                        total_deducciones[slip.employee_id.id] = va_dedu                            
                        code_col = 35
                else:
                        #IHSS    
            ...
            for record in record_ids:
                ...
                if record.code == 'SQ':
                    total_sueldo_quince += suelquincenall
                    record.write({
                        'amount': suelquincenall
                    })
                ...
                if record.code == 'PRESTA':
                    total_prestamo += to_descu4
                    record.write({
                        'amount': to_descu4
                    })
                if record.code == 'INCAPA':
                    total_incapa += to_descu5
                    record.write({
                        'amount': to_descu5
                    })
                if record.code == 'TELEFO':
                    total_telefonode += to_descu6
                    record.write({
                        'amount': to_descu6
                    })
                if record.code == 'CAFE1':
                    total_cafe1 += to_descu7
                    record.write({
                        'amount': to_descu7
                    })
                if record.code == 'LABO':
                    total_labora += to_descu8
                    record.write({
                        'amount': to_descu8
                    })
                if record.code == 'PRICE':
                    total_pricesmart += to_descu9
                    record.write({
                        'amount': to_descu9
                    })
                if record.code == 'UNIFO':
                    total_uniforme += to_descu10
                    record.write({
                        'amount': to_descu10
                    })
                if record.code == 'OTRAS':
                    total_otras_dedu += to_descu11
                    record.write({
                        'amount': to_descu11
                    })
                
                if record.code == 'NOTRABAJO':
                    total_no_trabajo += to_descu12
                    record.write({
                        'amount': to_descu12
                    })
            ...
            code_col = code_col + 1
            #PRESTAMO
            worksheet.write(row, code_col, total_prestamo, normal_num_bold)
            code_col = code_col + 1
            #INCAPACIDAD
            worksheet.write(row, code_col, total_incapa, normal_num_bold)
            code_col = code_col + 1
            #TELEFONO
            worksheet.write(row, code_col, total_telefonode, normal_num_bold)
            code_col = code_col + 1
            #CAFETERIA1
            worksheet.write(row, code_col, total_cafe1, normal_num_bold)
            code_col = code_col + 1
            #PRUEBA_LABO
            worksheet.write(row, code_col, total_labora, normal_num_bold)
            code_col = code_col + 1
            #PRICESMART
            worksheet.write(row, code_col, total_pricesmart, normal_num_bold)
            code_col = code_col + 1
            #UNIFORME
            ...
            code_col = code_col + 1
            #NO TRABAJO
            worksheet.write(row, code_col, total_no_trabajo, normal_num_bold)
            code_col = code_col + 1
            #TOTAL DEDUCCIONES
